...
- Header + **一条**筛选/搜索工具带 + DataTable + 空态。页头任选：Starter `h1`+`Breadcrumbs`+`Button`，或 Kit `PageTitleToolbar`（查 `/cases/toolbar`）。工具带任选：`ButtonGroup`+`TextField`，或 `Toolbar`+`ToolbarSearchInput`。禁止手搓页头/搜索。
- 新建按钮 → form dialog  
