...
function toApproval(row: ApprovalRequestRow): ApprovalRequest {
  ...
  return {
    ...
    decidedAt: row.decidedAt ? formatTime(row.decidedAt.toISOString()) : "—",
    created: formatTime(row.createdAt.toISOString()),
    updated: formatDateOnly(row.updatedAt.toISOString()),
  };
}
